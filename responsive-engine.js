...
function syncSelectedColor(){
 ...
 const btn=$('drawDot')?.closest('button');
 if(btn){btn.dataset.selectedColor=c;btn.setAttribute('aria-label','สีที่เลือก '+c)}
}
function sync(){
...
function hookColorPicker(){
 if(typeof window.pickColor==='function'&&!window.pickColor.__wadfunV6){
  const old=window.pickColor;
  const wrapped=function(c){const r=old.apply(this,arguments);setTimeout(syncSelectedColor,0);return r};
  wrapped.__wadfunV6=true;window.pickColor=wrapped;
 }
 if(typeof window.setHue==='function'&&!window.setHue.__wadfunV6){
  const old=window.setHue;
  const wrapped=function(){const r=old.apply(this,arguments);setTimeout(syncSelectedColor,0);return r};
  wrapped.__wadfunV6=true;window.setHue=wrapped;
 }
 if(typeof window.setShade==='function'&&!window.setShade.__wadfunV6){
  const old=window.setShade;
  const wrapped=function(){const r=old.apply(this,arguments);setTimeout(syncSelectedColor,0);return r};
  wrapped.__wadfunV6=true;window.setShade=wrapped;
 }
}
...
obs.observe(document.documentElement,{subtree:true,attributes:true,attributeFilter:['class','style','value']});
const timer=setInterval(()=>{if($('draw')||$('color')){hookColorPicker();fitAfterShow();}else hookColorPicker();},100);
