/* Wadfun Color Library V8 — dedicated picture-selection screen */
(function(){
'use strict';
...
function hideAllScreens(){document.querySelectorAll('.screen').forEach(s=>{s.classList.remove('active');s.style.setProperty('display','none','important')})}
function showOnly(id){const s=$(id);if(!s)return false;hideAllScreens();s.classList.add('active');s.style.setProperty('display','block','important');return true}
...
function showCategories(){renderCategories();showOnly('categories');grid()?.style.setProperty('display','grid','important');scrollTop();return true}
function showPicker(){renderPictures();showOnly('picker');pgrid()?.style.setProperty('display','grid','important');scrollTop();return true}
function selectedItem(){return data[activeCategory]?.items[selectedIndex]}
function directColorScreen(){const color=$('color');if(!color)return false;showOnly('color');try{window.initColor?.()}catch(e){console.warn('[Wadfun] initColor failed',e)}return true}
...
function install(){if(window.__wadfunColorLibraryV8Installed)return;window.__wadfunColorLibraryV8Installed=true;document.addEventListener('click',e=>{const cat=e.target.closest?.('#categoriesGrid .cat');if(cat){e.preventDefault();e.stopImmediatePropagation();activeCategory=cat.dataset.cat||'animals';selectedIndex=null;ensureState();showPicker();return}const pic=e.target.closest?.('#pickerGrid .template');if(pic){e.preventDefault();e.stopImmediatePropagation();choosePicture(Number(pic.dataset.index));return}if(e.target.closest?.('#wadfunColorCategoryBack')){e.preventDefault();e.stopImmediatePropagation();showCategories();return}if(e.target.closest?.('#wadfunChangePicture')){e.preventDefault();e.stopImmediatePropagation();changePicture();return}},true);addChangeButton()}
