...
async function saveCurrentArtwork(id){
  ...
  if(!c)c=activeCanvas();
  if(!c||typeof c.toDataURL!=='function')throw new Error('No active drawing canvas found');
...
async function handleFinish(){
  ...
  try{const saved=await saveCurrentArtwork(id);const c=id?document.getElementById(id):activeCanvas();if(c&&document.getElementById('finishImg'))document.getElementById('finishImg').src=c.toDataURL('image/png');if(typeof window.show==='function')window.show('finish');return saved}catch(e){console.error('[Wadfun] save failed',e);throw e}
}
function installFinishHook(){
  ...
  const wrapped=async function(){
    try{return await handleFinish.apply(this,arguments)}
    catch(e){console.error('[Wadfun] gallery save hook failed',e);return fn.apply(this,arguments)}
  };
