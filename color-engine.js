/* Wadfun Color Engine V13 — fast bucket fill + smooth freehand paint, protected line art. Draw mode untouched. */
(function(){
'use strict';
...
function fillRegion(x,y,hex){
 ...
 const p=rgb(hex),im=paintCtx.getImageData(0,0,w,h),d=im.data,seen=new Uint8Array(w*h),stack=[start];let changed=false;
 while(stack.length){const q=stack.pop();if(q<0||q>=w*h||seen[q]||mask[q])continue;seen[q]=1;const i=q*4;if(d[i]!==p[0]||d[i+1]!==p[1]||d[i+2]!==p[2]||d[i+3]!==255)changed=true;d[i]=p[0];d[i+1]=p[1];d[i+2]=p[2];d[i+3]=255;const qx=q%w;if(qx)stack.push(q-1);if(qx<w-1)stack.push(q+1);if(q>=w)stack.push(q-w);if(q<w*(h-1))stack.push(q+w)}
...
function drawSegment(x1,y1,x2,y2){
 const a=point(document.getElementById('colorCanvas'),x1,y1),b=point(document.getElementById('colorCanvas'),x2,y2),c=document.getElementById('colorCanvas'),m=mode();
 ...
 paintCtx.save();paintCtx.globalCompositeOperation=m==='eraser'?'destination-out':'source-over';paintCtx.drawImage(strokeCanvas,sx,sy,sw,sh,sx,sy,sw,sh);paintCtx.restore();requestRender(c)
}
function install(){
 const c=document.getElementById('colorCanvas'),v=document.getElementById('colorViewport'),z=document.getElementById('colorZoom');
 if(!c||!v||!z||c.dataset.colorV13)return false;c.dataset.colorV13='1';templateChanged();window.wadfunColorTemplateChanged=templateChanged;
 if(typeof window.undoColor!=='function'||!window.__wadfunUndoColorV13){window.__wadfunUndoColorV13=true;window.undoColor=restoreUndo}
 ...
 function start(x,y,id){if(mode()==='bucket')return false;saveUndo();active=true;pid=id;lastX=x;lastY=y;drawSegment(x,y,x,y);return true}
 function move(x,y){if(!active||pid===null)return;drawSegment(lastX,lastY,x,y);lastX=x;lastY=y}
 ...
 const wrap=window.loadTemplate;if(typeof wrap==='function'&&!window.__wadfunColorLoadWrapV13){window.__wadfunColorLoadWrapV13=true;window.loadTemplate=function(){const r=wrap.apply(this,arguments);setTimeout(templateChanged,0);return r}}
 return true
}
const timer=setInterval(()=>{if(install())clearInterval(timer)},100);
window.wadfunFillRegionV13=fillRegion;window.wadfunResetColorV13=templateChanged;
})();
