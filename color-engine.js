/* Wadfun Color Engine V13 — freehand strokes stay visible over filled regions */
(function(){
'use strict';
...
function ensureLineOverlay(c){
  if(lineOverlay&&lineW===c.width&&lineH===c.height){lineOverlay.style.zIndex='9999';return}
  const snapshot=window.wadfunGetColorTemplateSnapshot?.();
  if(!snapshot||snapshot.width!==c.width||snapshot.height!==c.height)return;
  const d=snapshot.data;
  const o=document.createElement('canvas');o.width=c.width;o.height=c.height;o.style.cssText='position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:9999!important;display:block!important';
  const od=o.getContext('2d'),out=od.createImageData(c.width,c.height);
  for(let i=0;i<d.length;i+=4){if(wall(d,i)){out.data[i]=d[i];out.data[i+1]=d[i+1];out.data[i+2]=d[i+2];out.data[i+3]=d[i+3]||255}}
  od.putImageData(out,0,0);
  const p=c.parentElement;if(p){if(getComputedStyle(p).position==='static')p.style.position='relative';c.style.position='relative';c.style.zIndex='1';if(lineOverlay)lineOverlay.remove();p.appendChild(o);lineOverlay=o;lineW=c.width;lineH=c.height}
}
function install(){
  const c=document.getElementById('colorCanvas'),v=document.getElementById('colorViewport'),z=document.getElementById('colorZoom');if(!c||!v||!z||c.dataset.colorV13)return false;c.dataset.colorV13='1';
  ...
  c.addEventListener('touchend',e=>{e.preventDefault();end()},{passive:false});c.addEventListener('touchcancel',end,{passive:false});return true
}
const timer=setInterval(()=>{if(install())clearInterval(timer)},100);window.wadfunFillRegionV13=fill;window.wadfunResetColorMask=resetMask;
})();
