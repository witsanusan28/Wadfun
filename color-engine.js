/* Wadfun Color Engine V5 — continuous region fill
   ใช้ Canvas Core สำหรับพิกัด/gesture และแยก flood fill แบบ boundary-only
*/
(function(){
'use strict';
...
function corePoint(c,x,y){return window.WadfunCanvasCore?.pointFromClient(c,x,y)||(()=>{const r=c.getBoundingClientRect();return{x:(x-r.left)*c.width/r.width,y:(y-r.top)*c.height/r.height}})()}
...
/*
  V5 เปลี่ยนหลักสำคัญ:
  ไม่ใช้ "สีเดิมของ pixel" เป็นเงื่อนไขแบ่งพื้นที่อีกต่อไป
  เพราะเส้น anti-alias / เงาอ่อน / compression ทำให้พื้นที่เดียวกัน
  ถูกตัดเป็นหลายช่อง
  ตอนนี้ใช้ "เส้นขอบ" เป็นกำแพง และให้พื้นที่ภายในเชื่อมต่อกันได้
*/
function fillRegion(c,x,y,hex){
  ...
  const paint=rgb(hex),seen=new Uint8Array(w*h),stack=new Int32Array(Math.min(w*h,1500000));let top=0;
  stack[top++]=y*w+x;

  while(top){
    const seed=stack[--top],sy=(seed/w)|0,sx=seed-sy*w;if(sx<0||sx>=w||sy<0||sy>=h)continue;
...
function install(){
  ...
  if(!c||!v||!z||c.dataset.colorV5)return;c.dataset.colorV5='1';
  const touches=new Map();let startDist=0,startScale=1,scale=1,lastTouch=0,moved=false;
  const distance=()=>{const a=[...touches.values()];if(a.length<2)return 0;return window.WadfunCanvasCore?.touchDistance([{clientX:a[0].x,clientY:a[0].y},{clientX:a[1].x,clientY:a[1].y}])||Math.hypot(a[0].x-a[1].x,a[0].y-a[1].y)};
  const applyZoom=s=>{scale=Math.max(1,Math.min(3,s));z.style.transform=`scale(${scale})`;window.wadfunZoomUI?.update('color',scale)};

  v.addEventListener('touchstart',e=>{for(const t of e.changedTouches)touches.set(t.identifier,{x:t.clientX,y:t.clientY});if(touches.size>=2){startDist=distance();startScale=scale;moved=true;e.preventDefault()}else{lastTouch=Date.now();moved=false}},{passive:false});
  v.addEventListener('touchmove',e=>{for(const t of e.changedTouches)if(touches.has(t.identifier))touches.set(t.identifier,{x:t.clientX,y:t.clientY});if(touches.size>=2&&startDist){const dd=distance();if(dd)applyZoom(startScale*dd/startDist);moved=true;e.preventDefault()}},{passive:false});
  const end=e=>{for(const t of e.changedTouches)touches.delete(t.identifier);if(touches.size<2)startDist=0};
  v.addEventListener('touchend',end,{passive:false});v.addEventListener('touchcancel',end,{passive:false});

  function doFill(x,y){if(document.getElementById('bucketBtn')&&!document.getElementById('bucketBtn').classList.contains('on'))return;const p=corePoint(c,x,y);const before=c.toDataURL();if(fillRegion(c,p.x,p.y,currentColor())){history.push(before);if(history.length>30)history.shift()}}
  ...
  c.addEventListener('touchend',e=>{if(touches.size||moved)return;const t=e.changedTouches[0];if(!t)return;if(Date.now()-lastTouch<=500)doFill(t.clientX,t.clientY)},{passive:false});
...
window.wadfunFillRegionV5=fillRegion;
})();
